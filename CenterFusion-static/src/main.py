...
from test import prefetch_test  # type: ignore
import json
...
def main(opt):
    ...
    if opt.eval:
        ...
        test_loader.dataset.run_eval(preds, opt.save_dir, n_plots=opt.eval_n_plots,
            ...
            with torch.no_grad():
                ...
                if opt.run_dataset_eval:
                    out_dir = val_loader.dataset.run_eval(preds, opt.save_dir,
                                                          ...
                                                          render_curves=0)
                    if os.path.exists(out_dir)==False: os.makedirs(out_dir, exist_ok=True)
...
# python main.py ddd --exp_id centerfusion --eval --shuffle_train --train_split train --val_split test
