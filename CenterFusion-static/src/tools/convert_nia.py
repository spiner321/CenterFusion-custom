...
DISTORTION =  True
DATA_PATH = '/data/kimgh/CenterFusion-custom/CenterFusion-static/data'
...
def main():
  ...
  for split in SPLITS: #SPLITS =  { 'train': 'train', 'val_norm': 'val_norm','val_abnorm': 'val_abnorm'}
    ...
    gloppath = f"{DATA_PATH}/{split}/source/*normal/??/*Clip*"
    scenes = glob.glob(gloppath)[:5]
    for scene in scenes:    
      ...
      # elif split[-6:] == 'abnorm':
      #     condition = 'abnormal'
      condition = Path(scene).parts[-3]
      class_ind = Path(scene).parts[-2]
