...
class nia(GenericDataset):
  ...
  def __init__(self, opt, split):
    # split_names = {
    #   'train': 'train_wabnorm',
    #   # 'train': 'val_abnorm_new2',
    #   'val_norm':'val_norm_new2',
    #   'val_abnorm': 'val_abnorm_new2',
    #   'test_norm': 'test_norm_new2',
    #   'test_abnorm': 'test_abnorm_new2',
    # }
    split_names = {
      'train': 'train',
      'val':'val',
    ...
    # data_dir = os.path.join(opt.data_dir, 'nia')
    ...
    self.images = None
    super(nia, self).__init__(opt, split, ann_path, data_dir)
  ...
  def convert_eval_format(self, results):
    ...
    for image_id in self.images:
      ...
      for item in results[image_id]:
        ...
        if 'size' in item:
          size = item['size'] # w l h
        else:
          size = [float(item['dim'][1]), float(item['dim'][2]), float(item['dim'][0])]
        if 'translation' in item:
          translation = item['translation']
        else:
          translation = np.array([item['loc'][0], item['loc'][1] - size[2], item['loc'][2], 1], np.float32)
        ...
        result = {
          ...
          'translation': [float(translation[0]), float(translation[2]), float(translation[1])],
          'size': size,
          'rotation': rotation,
          'velocity': velocity,
    ...
    for sample_token in ret['results'].keys():
      ...
      ret['results'][sample_token] = [ret['results'][sample_token][ind] for _, ind in confs[:min(500, len(confs))]]
  ...
  def save_results(self, results, save_dir, task, split):
    json.dump(self.convert_eval_format(results), 
                open('{}/results_nia_{}_{}.json'.format(save_dir, task, split), 'w'))


  def run_eval(self, results, save_dir, n_plots=10, render_curves=True):
    ...
    #   '--ann_path ./data/nia/annotations/{}.json'.format(split) + \
