...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--dataset', default='nia',
                             help='see lib/dataset/dataset_facotry for ' + 
                            'available datasets')
    self.parser.add_argument('--test_dataset', default='nia',
                             help='coco | kitti | coco_hp | pascal')
    self.parser.add_argument('--exp_id', default='centerfusion')
    ...
    self.parser.add_argument('--data_dir', type=str, default=False, help='set data dir')
    self.parser.add_argument('--save_dir', type=str, default=False, help='set save dir')
    ...
    self.parser.add_argument('--lr', type=float, default=2.5e-4, #2.5e-4
                             help='learning rate for batch size 32.')
    self.parser.add_argument('--lr_step', type=str, default='10,20',
                             help='drop learning rate by 10.')
    self.parser.add_argument('--save_point', type=str, default='10,15,20',
    ...
    self.parser.add_argument('--nuscenes_att', action='store_false')
    self.parser.add_argument('--nuscenes_att_weight', type=float, default=0)
    self.parser.add_argument('--velocity', action='store_false')
    self.parser.add_argument('--velocity_weight', type=float, default=0)
    ...
    self.parser.add_argument('--max_pc_dist', type=float, default=60.0,
    ...
    self.parser.add_argument('--pillar_dims', type=str, default='1.0,0.2,0.2',
  ...
  def parse(self, args=''):
    ...
    # log dirs
    if opt.data_dir:
      opt.exp_dir = opt.save_dir
      opt.debug_dir = os.path.join(opt.save_dir, 'debug')

    else:
      opt.root_dir = os.path.join(os.path.dirname(os.getcwd()))
    ...
    if opt.pointcloud:
      ...
      opt.secondary_heads = ['dep_sec', 'rot_sec']
      # opt.hm_dist_thresh = {
      #   'car': 0, 
      #   'truck': 0,
      #   'bus': 0,
